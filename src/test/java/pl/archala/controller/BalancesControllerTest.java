...
import static pl.archala.utils.EndpointProvider.API_BALANCES_TRANSACTION;
...
class BalancesControllerTest extends PostgresqlContainer {
    ...
    void shouldCreateBalanceForUser() {
        ...
        //when
        webTestClient.post().uri(API_USERS_REGISTER).bodyValue(addUserDTO).exchange()
    ...
    void shouldThrowExceptionIfUserSendMoneyWithoutCreatedBalance() {
        ...
        //when
        webTestClient.post().uri(API_USERS_REGISTER).bodyValue(addUserDTO1)
                .exchange()
                .expectStatus().isCreated();

        ErrorResponse errorResponse = webTestClient.post().uri(API_BALANCES_TRANSACTION).bodyValue(transactionDTO)
                .headers(headers -> headers.setBasicAuth(username, password)).exchange()
                .expectStatus().isBadRequest()
                .expectBody(ErrorResponse.class)
                .returnResult().getResponseBody();
    ...
    void shouldIncrementDailyTransactionsCountAndThrowExceptionIfWeExceedTheLimit() {
        ...
        webTestClient.post().uri(API_USERS_REGISTER).bodyValue(addUserDTO2)
                .exchange()
                .expectStatus().isCreated();
        ...
        //when
        GetBalanceDTO getSourceBalanceDTOAfterOneTransaction = webTestClient.post().uri(API_BALANCES_TRANSACTION).bodyValue(transactionDTO)
                .headers(headers -> headers.setBasicAuth(username1, password)).exchange()
                .expectStatus().isOk()
                .expectBody(GetBalanceDTO.class)
                .returnResult().getResponseBody();

        GetBalanceDTO getSourceBalanceDTOAfterTwoTransactions = webTestClient.post().uri(API_BALANCES_TRANSACTION).bodyValue(transactionDTO)
                .headers(headers -> headers.setBasicAuth(username1, password)).exchange()
                .expectStatus().isOk()
                .expectBody(GetBalanceDTO.class)
                .returnResult().getResponseBody();

        GetBalanceDTO getSourceBalanceDTOAfterThreeTransactions = webTestClient.post().uri(API_BALANCES_TRANSACTION).bodyValue(transactionDTO)
                .headers(headers -> headers.setBasicAuth(username1, password)).exchange()
                .expectStatus().isOk()
                .expectBody(GetBalanceDTO.class)
                .returnResult().getResponseBody();
    ...
    void shouldThrowExceptionIfValueToSendIsInvalid() {
        ...
        //when
        ErrorResponse zeroValueError = webTestClient.post().uri(API_BALANCES_TRANSACTION).bodyValue(transactionDTO1)
                .headers(headers -> headers.setBasicAuth(username1, password)).exchange()
                .expectStatus().isBadRequest()
                .expectBody(ErrorResponse.class)
                .returnResult().getResponseBody();

        ErrorResponse moreThanTwoDigitsAfterCommaError = webTestClient.post().uri(API_BALANCES_TRANSACTION).bodyValue(transactionDTO2)
                .headers(headers -> headers.setBasicAuth(username1, password)).exchange()
                .expectStatus().isBadRequest()
                .expectBody(ErrorResponse.class)
                .returnResult().getResponseBody();

        ErrorResponse moreThanTenDigitsBeforeCommaError = webTestClient.post().uri(API_BALANCES_TRANSACTION).bodyValue(transactionDTO3)
                .headers(headers -> headers.setBasicAuth(username1, password)).exchange()
                .expectStatus().isBadRequest()
                .expectBody(ErrorResponse.class)
                .returnResult().getResponseBody();
